...
Transport_layer::serial_port_error_code_t Application_layer::get_serial_port_error_code(){
    if(request_sent_ == false){
        return Transport_layer::serial_port_error_code_t::NO_REQUEST_SENT;
    }
    return static_cast<Transport_layer::serial_port_error_code> (transport_layer_error_);
}
...
void Command_homologation_start_continuous_mode::request_cmd(void) {
    std::cout << __PRETTY_FUNCTION__ << std::endl;
    std::vector<uint8_t> data = {0x03};
    data.insert(data.end(), 4); // start continuous mode
    data.insert(data.end(), 2);     //freq
    data.insert(data.end(), 0); // modulation on
    Application_layer::request_cmd(data);
}
...
void Command_homologation_stop_auto_tx::request_cmd(void) {
    std::cout << __PRETTY_FUNCTION__ <<std::endl;
    std::vector<uint8_t> data = {0x03};
    data.insert(data.end(), 9); // start auto tx
    Application_layer::request_cmd(data);
}
